...
type ResourceSyncer interface {
	...
	GetObservedState(ctx context.Context, obj *v1alpha1.Object, current *unstructured.Unstructured) (*unstructured.Unstructured, error)
	...
	GetDesiredState(ctx context.Context, obj *v1alpha1.Object, manifest *unstructured.Unstructured) (*unstructured.Unstructured, error)
...
func Setup(mgr ctrl.Manager, o controller.Options, sanitizeSecrets bool, stripManagedFields bool, pollJitterPercentage uint, legacyCSAFieldManagers []string) error { // nolint:gocyclo // Too many branches due to alpha features, hopefully we can clean them up after we graduate them.
	...
	if o.Features.Enabled(features.EnableAlphaWatches) {
		...
		if err := ca.IndexField(context.Background(), &v1alpha1.Object{}, resourceRefsIndex, IndexByProviderNamespacedNameGVK); err != nil {
	...
	if err := mgr.Add(statemetrics.NewMRStateRecorder(
		mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1alpha1.ObjectList{}, o.MetricOptions.PollStateMetricInterval)); err != nil {
		return err
	}
...
func SetupGated(mgr ctrl.Manager, o controller.Options, sanitizeSecrets bool, stripManagedFields bool, pollJitterPercentage uint, legacyCSAFieldManagers []string) error {
	o.Gate.Register(func() {
		if err := Setup(mgr, o, sanitizeSecrets, stripManagedFields, pollJitterPercentage, legacyCSAFieldManagers); err != nil {
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	e := &external{
		...
		stripManagedFields: c.stripManagedFields,
...
func parseManifest(obj *v1alpha1.Object) (*unstructured.Unstructured, error) {
...
func (c *external) checkDeriveFromCelQuery(obj *v1alpha1.Object, observed *unstructured.Unstructured) (ready bool, err error) {
...
func (c *external) shouldWatch(cr *v1alpha1.Object) bool {
