...
type (
	kubernetesObjectModifier func(obj *objv1alpha1.Object)
	externalResourceModifier func(res *unstructured.Unstructured)
)

func kubernetesObject(om ...kubernetesObjectModifier) *objv1alpha1.Object {
	o := &objv1alpha1.Object{
		TypeMeta: metav1.TypeMeta{
			APIVersion: objv1alpha1.SchemeGroupVersion.String(),
			Kind:       objv1alpha1.ObjectKind,
		},
		...
		Status: objv1alpha1.ObjectStatus{},
	}
...
func objectReferences() []objv1alpha1.Reference {
...
func TestAddFinalizer(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"EmptyReference": {
			args: args{
				mg: kubernetesObject(func(obj *objv1alpha1.Object) {
					obj.Spec.References = []objv1alpha1.Reference{{}}
				}),
				client: resource.ClientApplicator{
					Client: &test.MockClient{
						MockUpdate: test.NewMockUpdateFn(nil),
...
func TestRemoveFinalizer(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if _, ok := tc.args.mg.(*objv1alpha1.Object); ok {
...
func TestConnectionDetails(t *testing.T) {
	...
	connDetail := objv1alpha1.ConnectionDetail{
		ObjectReference: corev1.ObjectReference{
			Kind:       "Secret",
			Namespace:  testNamespace,
			Name:       testSecretName,
	...
	type args struct {
		kube        client.Client
		connDetails []objv1alpha1.ConnectionDetail
	}
	type want struct {
	...
	cases := map[string]struct {
		...
		"Fail_ObjectNotExisting": {
			args: args{
				...
				connDetails: []objv1alpha1.ConnectionDetail{connDetail},
			},
			want: want{
				out: managed.ConnectionDetails{},
				...
				obj: &objv1alpha1.Object{
					Spec: objv1alpha1.ObjectSpec{
						Readiness: objv1alpha1.Readiness{
							Policy: objv1alpha1.ReadinessPolicySuccessfulCreate,
						},
					},
				},
				observed: &unstructured.Unstructured{
...
func TestSetAtProviderRemovesManagedFields(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &external{
				logger:             logging.NewNopLogger(),
				stripManagedFields: tc.stripManagedFields,
			}
